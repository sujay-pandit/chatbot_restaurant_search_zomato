...
class Zomato:
    ...
    def check_city_valid(self,query=""):
      """
      Takes either query, latitude and longitude or cuisine as input.
      Returns a list of Restaurant IDs.
      """
      headers = {'Accept': 'application/json', 'user-key': self.user_key}
      r = (requests.get(base_url + "locations?query=" + str(query) + "&count=" + str(1), headers=headers).content).decode("utf-8")
      d1 = json.loads(r)
      if(len(d1["location_suggestions"])):
        if(d1["location_suggestions"][0]['country_name'].lower()=='india'):
          return (True,True)
        else:
          return (True,False)
      else:
